...
database.ref("/").on("value", (snapshot) => {
    ...
    if (data.System_Lock !== undefined) {
        ...
        if (overlay) {
            overlay.style.display = data.System_Lock ? "flex" : "none";
        }
    }
...
function executeSchedule(task) {
    // Perform the action (Simulate clicking the device if state doesn't match)
    const card = document.getElementById(task.device + "-card");
    if(card) {
        const isActive = card.classList.contains("active");
        if(task.action === "on" && !isActive) {
            toggleDevice(task.device);
        } else if(task.action === "off" && isActive) {
            toggleDevice(task.device);
        }
    }
    
    // Auto-delete the schedule
    deleteSchedule(task.id, false);
}
...
function triggerLightAuto(e) {
    if(e) e.stopPropagation();
    const isOffline = (Math.floor(Date.now() / 1000) - lastHeartbeat) > 60;
    if (isOffline) { showToast("Error: System is Offline!"); return; }
    
    database.ref("/outsideLightAuto").set(true).catch(e => showToast("Error: " + e.message));
    showToast("Command sent: Auto Mode");
}



function triggerLightForce(e) {
    if(e) e.stopPropagation();
    const isOffline = (Math.floor(Date.now() / 1000) - lastHeartbeat) > 60;
    if (isOffline) { showToast("Error: System is Offline!"); return; }
